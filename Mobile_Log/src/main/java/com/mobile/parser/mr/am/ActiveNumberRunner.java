...
public class ActiveNumberRunner implements Tool {
    private static final Logger logger = Logger.getLogger(ToHdfsRunner.class);

    public static void main(String[] args) {
        try {
            ToolRunner.run(new Configuration(), new ActiveNumberRunner(), args);
        } catch (Exception e) {
    ...
    public int run(String[] args) throws Exception {
        ...
        setArg(args, conf);
        //获取job
        Job job = Job.getInstance(conf, "active number");

        job.setJarByClass(ActiveNumberRunner.class);
        job.setMapperClass(ActiveNumberMapper.class);
        job.setMapOutputKeyClass(StatusUserDimension.class);
        job.setMapOutputValueClass(TimeOutputWritable.class);

        job.setReducerClass(ActiveNumberReducer.class);
