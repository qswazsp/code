...
//        job.setOutputFormatClass(SqlFormat.class);
    ...
    private void computeTotalUser(Job job) {
        ...
        Map<String, Integer> info2 = new ConcurrentHashMap<>();
        ...
        try {
            if (yesterDimensionId > 0) {
                pre = conn.prepareStatement("select platform_dimension_id , total_members from stats_user where date_dimension_id = ?");
                pre.setInt(1, yesterDimensionId);
                rs = pre.executeQuery();
                while (rs.next()) {
                    info.put(rs.getInt("platform_dimension_id") + "", rs.getInt("total_members"));
                }
            }
            //查询今天的数据
            if (todayDimensionId > 0) {
                pre = conn.prepareStatement("select platform_dimension_id , new_members from stats_user where date_dimension_id = ?");
                pre.setInt(1, todayDimensionId);
                rs = pre.executeQuery();
                while (rs.next()) {
                    //遍历每一个platform加到totalInstallUser中
                    int platformID = rs.getInt("platform_dimension_id");
                    int totalInstallUser = rs.getInt("new_members");
